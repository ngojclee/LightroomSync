...
- Wave 6 smoke baseline is now implemented with `scripts/e2e_wails_ui_smoke.ps1`, providing reproducible startup/IPC/close evidence for Wails runtime. Strict Wails runtime validation passes successfully.
- Wave 6 tray validation tooling is now runtime-aware: `scripts/e2e_tray_ui_smoke.ps1` supports `-UIRuntime wails` and successfully passed focus evidence capture. `cmd/ui` now enforces single-instance/focus flow before runtime dispatch for both harness and wails modes.
